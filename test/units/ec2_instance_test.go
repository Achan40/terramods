...
func TestEC2InstanceCustomVPC(t *testing.T) {
	...
	// Step 4: deploy EC2 into the VPC (no Tailscale auth key)
	...
	// Source/dest check enabled (no Tailscale auth key provided)
	require.NotNil(t, instance.SourceDestCheck)
	assert.True(t, *instance.SourceDestCheck)
}

// TestEC2SubnetRouter deploys an EC2 instance with a Tailscale auth key and
// validates that source/dest check is disabled at the AWS level.
func TestEC2SubnetRouter(t *testing.T) {
	...
	iamOptions := &terraform.Options{
		...
		Vars: map[string]interface{}{
			"region":    region,
			"role_name": "test-router-" + uniqueID,
		},
	}
	...
	ec2Options := &terraform.Options{
		...
		Vars: map[string]interface{}{
			"region":                        region,
			"instance_name":                 "test-router-" + uniqueID,
			"ami_id":                        "ami-0900fe555666598a2",
			"instance_type":                 "t3.micro",
			"instance_count":                1,
			"vpc_id":                        vpcID,
			...
			"tailscale_auth_key":            "tskey-test-placeholder",
		},
	}
	...
	// Source/dest check must be disabled when a Tailscale auth key is provided
	require.NotNil(t, instance.SourceDestCheck)
	assert.False(t, *instance.SourceDestCheck)
}
